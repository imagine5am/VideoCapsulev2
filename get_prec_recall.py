...
def get_precision_recall():
    ...
    with tf.Session(graph=capsnet.graph, config=config.gpu_config) as sess:
        ...
        data_gens = {'Minetto_Gen': Minetto_Gen(data_queue_length=3, sec_to_wait=20),
                     'ICDAR_Gen': ICDAR_Gen(split_type='test', data_queue_length=10, sec_to_wait=20),
                     'YVT_Gen': YVT_Gen(split_type='test', data_queue_length=8, sec_to_wait=20)
                    }
        for data_gen_name, data_gen in data_gens.items():
            tp, fn, fp = {}, {}, {}
